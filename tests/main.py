...
def test_second_task(driver):
    time.sleep(3)
    MainPage(driver).click_products_buy([0, 1])
    MainPage(driver).click(MainPage.BUTTON_CART)
    time.sleep(1)
...
def test_third_task(driver):
    time.sleep(3)
    MainPage(driver).click(MainPage.DROPDOWN_PC)
    MainPage(driver).click(MainPage.LI_PC)
    time.sleep(1)
...
def test_fifth_task(driver):
    time.sleep(3)
    MainPage(driver).input_search("random text!")
...
def test_second_dop_task(driver):
    ...
    ProductPage(driver).click(ProductPage.SELECT_COLOR)
    ProductPage(driver).downArrow().downArrow()
    ProductPage(driver).enter()
...
@allure.title("Add telephone iPhone to cart")
def test_forth_dop_task(driver):
    time.sleep(3)
    MainPage(driver).click(MainPage.DROPDOWN_TELEPHONES)
    MainPage(driver).click(MainPage.PRODUCT_IPHONE)
    ProductPage(driver).click(ProductPage.BUTTON_BUY)
